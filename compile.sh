...
APPLICATION_VERSION=2.8.38
APPLICATION_VERSION_STRING=2.8.4_alpha1
...
do_checks() {
    find "${DESTDIR}" -type f | while read file ; do
        if /usr/bin/file "${file}" | grep -q "Mach-O" ; then
            ...
            if nm -arch x86_64 -m "${file}"  | grep -v "_voucher" | grep -v "darwin_check_fd_set_overflow" | grep -v "_pthread_mutexattr" | grep -v "from libc++" | grep -v "from libobjc" | grep -q "(undefined) weak external" ; then
...
do_meson_build src/xorg/lib/libxcvt ${ARCHS_LIB}
